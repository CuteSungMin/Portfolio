...
function Section1({onInputChange, inputValues}) {
  ...
  const tableData = [
    ...
    {tableTitle : Sec1Data.Title.greetings, tableText : (<span>안녕하세요 GUARD에서 FRONT-END Developer가 된 CSM 입니다.<br />CSM의 사명은 도전을 무서워 하지 말자 입니다.<br />사업을 바꾸는 도전을 시작한지 어느덧 8개월이 되었습니다.<br />꾸준히 자기개발을 통해 성장하는 Developer가 되겠습니다</span>)},
  ]
  const yearNumData = [
      ...
      <article className="w1500 section1Article">
        ...
        <div className="section1Right">
          ...
          {yearTextData.map((textData,index)=>(
              <p key={index}>{textData.yearText}</p>
            ))}
          </div>
        </div>
      </article>
